...
import pytest
from agentdag import DAG, Blackboard, Scheduler
...
def test_llm_coder_passes_adversary_failures_in_prompt():
    stub = _StubLLM("```python\ndef solve(s):\n    return False\n```")
    _run(LLMCoder(stub), adversary_failures=[{"input": ")(", "expected": False, "got": True}])
    assert len(stub.calls) == 1
...
def test_llm_coder_uses_not_run_default_for_missing_got():
    stub = _StubLLM("```python\ndef solve(s):\n    return True\n```")
    _run(LLMCoder(stub), adversary_failures=[{"input": "x", "expected": False}])  # no 'got'
    p = stub.calls[0]["prompt"]
    assert "<not-run>" in p
...
def test_llm_coder_shortens_long_adversarial_input():
    long_input = "(" * 1000
    stub = _StubLLM("```python\ndef solve(s):\n    return True\n```")
    _run(LLMCoder(stub), adversary_failures=[{"input": long_input, "expected": False, "got": True}])
    p = stub.calls[0]["prompt"]
