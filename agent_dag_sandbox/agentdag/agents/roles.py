...
def _safe_globals() -> Dict[str, Any]:
    ...
    safe = {name: getattr(_b, name) for name in _SAFE_BUILTIN_NAMES
            if hasattr(_b, name)}
...
class LLMCoder:
    ...
    def __init__(self, llm: LLMClient, entrypoint: str = "solve",
                 fallback: "Coder | None" = None):
        self.llm = llm
    ...
    def __call__(self, ctx: NodeContext) -> Dict[str, Any]:
        ...
        try:
            exec(compile(source, "<llm-probe>", "exec"),
                 _safe_globals(), probe_ns)
        ...
        if not callable(fn):
            return self._fallback(
                ctx, reason=f"entrypoint-{self.entrypoint!r}-not-callable"
            )
...
def _run_cases(
    ...
    try:
        ...
        return [{"input": None, "expected": None,
                 "got": "<not-run>", "error": f"compile: {e!r}"}]
