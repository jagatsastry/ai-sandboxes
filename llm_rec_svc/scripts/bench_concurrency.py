...
def one_call(client: httpx.Client, url: str, profile: str,
             cands: int, topk: int) -> dict:
    t0 = time.perf_counter()
    r = client.post(url, json={"profile": profile,
                               "top_k": topk, "n_candidates": cands})
    r.raise_for_status()
...
def run_level(url: str, concurrency: int, reqs: int,
              cands: int, topk: int) -> dict:
    """Fire `reqs` requests with `concurrency` workers in flight at once."""
    lats = {"wall": [], "total": [], "gpu": [], "embed": [],
            "tokenize": [], "candidate": []}
...
def main():
    ...
    ap.add_argument("--reqs", type=int, default=16,
                    help="requests per concurrency level (after warmup)")
    ...
    rows = []
    for level in levels:
        print(f"\n>> concurrency={level}  reqs={args.reqs}  "
              f"cands={args.cands}  topk={args.topk}")
        r = run_level(args.url, level, args.reqs, args.cands, args.topk)
        rows.append(r)
        L = r["lats"]
        print(f"   thru={r['throughput_rps']:.2f} rps   "
    ...
    if len(rows) >= 2:
        ...
        print()
        print(f"From c={base['concurrency']} to c={last['concurrency']} "
        ...
        if ratio_thru < 1.3 and ratio_lat > 1.5:
            print("  -> classic single-worker serialization. Concurrency does "
